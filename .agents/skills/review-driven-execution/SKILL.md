...
8. 작업 종료 직전 Completion Context Triage를 수행합니다.
   - 남아 있는 큐(미해결 요청, 후속 task/todo, 실패 검증, 미정리 상태)를 확인합니다.
   - context 유지 이점이 작으면 정제 문서만 남기고 raw 문맥은 더 끌고 가지 않습니다.
   - 후속 작업이 있으면 현재 상태, 핵심 파일, 검증 결과, 잔여 리스크, 다음 액션만 압축해 유지합니다.
   - 이 판단이 오히려 노이즈를 늘리면 적용하지 않습니다.
...
- Completion Context Triage는 행동 지침으로만 유지하고, deterministic hook로 강제하지 않습니다.
