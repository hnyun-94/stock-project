...
- 전역 정책은 환경변수 기반 상수로 두고, 기본값은 60초로 고정하는 것이 운영 제어에 유리합니다.
...
- 6초, 10초처럼 짧은 retryDelay는 기다리는 편이 운영 품질에 유리합니다.
...
- 채택 1: Gemini 429 quota 응답의 `RetryInfo.retryDelay`를 읽고, 허용 범위 내에서는 한 번 기다렸다가 재시도
- 채택 2: `retryDelay + 현재까지 경과 시간 <= 60초`일 때만 지연 재시도
