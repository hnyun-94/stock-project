...
25. 2026-03-08 지수/지표 텍스트 시각화 보강
- 이메일 경로에 맞춰 신뢰도, 시장 심리, 데이터 품질, 보조 지표 변화에 텍스트 기반 미니 시각화(게이지, 성공률 바, 방향 아이콘, 3포인트 스파크라인)를 추가
- 외부 이미지나 JS 차트 없이 기존 Markdown/HTML 렌더링 구조 안에서 스캔성을 높이도록 구현
- 관련 payload/markdown 회귀 테스트를 보강해 시각화 문자열이 유지되도록 고정
